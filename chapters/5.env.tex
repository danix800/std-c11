...
\paragraph{}
实现在两个数据处理系统环境中翻译C源文件并执行C程序，本国际标准中称为\textit{翻译
环境}和\textit{执行环境}。它们的特性定义并约束执行根据合规实现的语法和语义规则所
构造的合规C程序的结果。
...
要求存储为文件，也不需要这些实体和任何外部表示之间存在一一对应的关系。描述只是概
念性的，并不指定任何特定的实现。}
\begin{enumerate}
  ...
  \tpit{源文件被分解为预处理标记\footnote{如\ref{lang.lex}所述，将源文件的字符划
    分为预处理标记的过程是上下文相关的。例如，请参见在\tm{\#include}预处理指令中
    对\tm{<}的处理。} 和空白字符序列（包括注释）。源文件不得以部分预处理标记或部
    分注释结尾。每个注释由一个空格字符替换。保留新行字符。除新行之外的每个非空空
    白字符序列是保留还是替换为一个空格字符由实现定义。}
...
实现定义，合规实现也应至少生成一条诊断信息（以实现定义的方式标识）。在其他情况下
...
\begin{lstlisting}
    ...
    int i;
\end{lstlisting}
因为在这些情况下，如果本国际标准中将结构的行为描述为既是约束错误，又会导致未定义
行为，则应诊断为约束错误。
...
\paragraph{}
在自由式环境中（在这种环境中C程序可能没有任何操作系统的帮助下执行），程序启动时
...
\begin{itemize}
  ...
  \item{如果\texttt{argc}的值大于$0$，\texttt{argv[0]}指向的字符串表示
    \textit{程序名}；如果程序名在主机环境中不可用，则\texttt{argv[0][0]}应该为零
...
$A$不是前序或后序于$B$，则$A$和$B$无序。当$A$前序或后序于$B$但未指明哪一个时，则
计算$A$和$B$为不确定性有序。\footnote{无序计算的执行可以交错进行。不确定性有序计
算不能交错，但可以任何顺序执行。} 表达式$A$和$B$之间存在\textit{序列点}意味着与
...
示无关。例如，不允许寄存器的隐式溢出更改值。此外，还需要显式存储和加载来舍入到存
储类型的精度。特别是，强制转换和赋值需要执行其指定的转换。对于片断
\begin{lstlisting}
    double d1, d2;
    float f;
    d1 = f = expression;
...
没有上溢和下溢的情况下，该实现通常也不能应用加法或乘法的数学结合律，也不能应用分
...
不存在其他三字母序列。未开始以上列出的三字母序列的每一个\tm{?}不变。
