...
全声明子中，声明子的嵌套序列中存在指定可变长度数组类型的声明子，则称全声明符指定
...
如果在声明``\tm{T D1}''中，\tm{D1}形如                                        \\
\mbox{\hspace{4em}\tm{(} \nt{identifier} \tm{)}}                              \\
\textit{ident}具有由声明``\tm{T D}''所指定的类型。因此，括号中的声明符与无括号声
明符相同，但复杂声明符的绑定可以用括号改变。

\implimit
\paragraph{}
如\ref{env.env.limit.trans}所述，实现可以限制直接或通过一个或多个\tm{typedef}修
改算术、结构、联合或\tm{void}类型的指针、数组和函数声明符的数量。
...
址来生成\tm{int}。在声明符\tm{(*pfi)()}中，需要额外的括号来指示通过指向函数的指针
...
\constraint
\paragraph{}
函数定义中声明的标识符（函数名）应具有函数类型，由函数定义的声明符部分指定。
...
符串文字，其中月份名称与\tm{ascTime}函数生成的名称相同，如果值小于10，则\tm{dd}
的第一个字符为空格字符。如果翻译日期不可用，应提供实现定义的有效日期。}
\mlst{\_\_FILE\_\_}{当前源文件的推定名称（字符串文字）。\footnotemark}
