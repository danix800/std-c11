...
\newcommand{\itm}[1]{\stepcounter{itmcntr}\item{#1
  ...
  \itm{赋值运算符中操作数的求值顺序（\ref{lang.expr.assign}）。}
  \itm{分配用于存储位域的可寻址存储单元的对齐（\ref{lang.dcl.type.struct}）。}
...
\newcommand{\itm}[2]{\stepcounter{itmcntr}\item{#1（#2）。
  ...
  {\ref{lang.concept.repr.int}}
  \itm{同一对象或函数的两个声明指定不兼容对象}{\ref{lang.concept.compat}}
  \itm{程序需要变长数组类型的兼容类型形式，该数组的大小由不求值表达式指定}
  ...
  \itm{求值时左值不指代对象}{\ref{lang.conv.other.desig}}
  ...
  \itm{表达式求值时出现异常条件}{\ref{lang.expr}}
  ...
  越过同一数组对象的结果，而该结果被用作一元运算符\tm{*}的操作数，且该运算符被求
  值}{\ref{lang.expr.add}}
...
\begin{itemize} % 11
  ...
  \item{由\tm{FLT\_EVAL\_METHOD}的非标准负值所刻画的求值方法
    （\ref{env.env.limits.num.float}）。}
...
\begin{itemize} % 7
  \item[\textbf{1}\hspace{5pt}---]{``普通''\tm{int}位域是作为\tm{signed int}还是
    作为\tm{unsigned int}位域（\ref{lang.dcl.type}，
    \ref{lang.dcl.type.struct}）。}
  \item{除\tm{\_Bool}，\tm{signed int}和\tm{unsigned int}外允许的位域类型
    （\ref{lang.dcl.type.struct}）。}
...
\newcommand{\itm}[2]{\stepcounter{itmcntr}\item{#1（#2）。
  ...
  \tm{math\_errhandling \& MATH\_ERRNO}非零时\tm{errno}是否设置为宏\tm{ERANGE}的
  值，以及当整型表达式\tm{math\_errhandling \& MATH\_ERREXCEPT}非零时是否引发
  ``下溢''浮点异常}{\ref{lib.math.error}}
